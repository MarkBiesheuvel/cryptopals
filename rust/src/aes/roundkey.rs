...
const ROUND_CONSTANT: [u8; 10] = [1, 2, 4, 8, 16, 32, 64, 128, 27, 54];

fn sub_byte(input: u8) -> u8 {
    S_BOX[input as usize]
}

fn round_constant(i: usize) -> u8 {
    ROUND_CONSTANT[i-1]
}
...
impl Roundkey {
    fn new(initial_key: [u8; 16]) -> Roundkey {
...
impl TryFrom<&str> for Roundkey {
    ...
    fn try_from(value: &str) -> Result<Self, Self::Error> {
        ...
        let mut initial_key = [0u8; 16];

        for (k, v) in initial_key.iter_mut().zip(value.as_bytes().into_iter()) {
            *k = *v;
        }
